...
-- Atomic sale creation function with row-level locking to prevent race conditions
-- This replaces the multi-step client-side sale creation with a single transaction
...
DECLARE
  ...
  v_product_id UUID;
  v_quantity INT;
  v_branch_stock_qty INT;
  v_product_stock_qty INT;
  v_available INT;
  v_product_name TEXT;
  v_new_qty INT;
...
BEGIN
  ...
  LOOP
    ...
    IF v_product_id IS NULL THEN
      CONTINUE; -- skip non-product items (custom/misc)
    END IF;

    -- Lock the branch_stock row (or product row) with FOR UPDATE to serialize concurrent access
  ...
  -- Step 3: All stock validated — create the sale record
  INSERT INTO sales (
    ...
    p_discount_type, p_discount_reason, p_discount_approved_by, COALESCE(p_rewards_redeemed_value, 0)
  )
  RETURNING id INTO v_sale_id;

  -- Step 4: Insert sale items and deduct stock atomically
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
  LOOP
    v_product_id := (v_item->>'product_id')::UUID;
    v_quantity := (v_item->>'quantity')::INT;
    ...
    IF v_product_id IS NOT NULL THEN
      IF p_branch_id IS NOT NULL THEN
        ...
        -- Recalculate global product stock as SUM of all branches
      ...
      ELSE
        -- Deduct global product stock directly
        UPDATE products
  ...
  -- Step 5: Return the created sale info
